...
- **Trigger**: 280ms hold on any player triggers the menu (movement threshold of 18px cancels if dragging)
...
- **Menu Structure**: Three-level horizontal cascade, centered below cursor:
  - Level 1: Pass, Run, Block (vertical stack, arrows indicate submenus) - role="menuitem" for accessibility
  - Level 2: Straight, Curved (expands right on hover) - role="menuitem" for accessibility
  - Level 3: Motion? and Make Primary? checkboxes (only for Pass/Run, not Block)
- **Menu Container**: role="menu" with data-testid="long-press-menu" for testing
- **Route Start**: Clicking Straight/Curved stores selection, then hover over player to start drawing
...
- **Race Condition Fix**: Uses `menuOpenRef` synchronous ref to coordinate timer callback with mouseup handlers, preventing the menu from closing before React can render it

**Player Interaction Patterns**:
- Quick tap: Select player (cyan ring)
- Double tap: Edit player label (2-character max)
- Long-press (280ms): Open Route Options menu (orange pulsing ring)
- Drag: Move player position (18px movement threshold)
