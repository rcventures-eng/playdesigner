...
The core play design functionality utilizes **HTML5 Canvas** and SVG for rendering. A central `FIELD` configuration object defines all field geometry (e.g., `FIELD.WIDTH`, `FIELD.HEIGHT`, `FIELD.PIXELS_PER_YARD`), ensuring consistent scaling and layout. The field is vertically oriented (694×512px) with a 60px header for metadata. The field was extended by 10 yards (120px) at the top to accommodate deep defensive zones. Key features include:
...
-   **Export Functionality**: Uses `html-to-image` to generate downloadable PNGs. Exports at native size (694x512) maintain pixel-perfect quality, while other sizes are generated by capturing at 2x resolution and downscaling with high-quality smoothing for crisp results.
