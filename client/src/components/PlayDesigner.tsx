...
export default function PlayDesigner({ isAdmin, setIsAdmin, showSignUp, setShowSignUp }: PlayDesignerProps) {
  ...
  const [advancedExpanded, setAdvancedExpanded] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('playDesigner_advancedExpanded') === 'true';
    }
    return false;
  });
  const [expertExpanded, setExpertExpanded] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('playDesigner_expertExpanded') === 'true';
    }
    return false;
  });
  ...
  // Persist progressive disclosure state to localStorage
  useEffect(() => {
    localStorage.setItem('playDesigner_advancedExpanded', String(advancedExpanded));
  }, [advancedExpanded]);
  
  useEffect(() => {
    localStorage.setItem('playDesigner_expertExpanded', String(expertExpanded));
  }, [expertExpanded]);
      ...
      if (!isNaN(playId)) {
        ...
        fetch(`/api/plays/${playId}`, { credentials: 'include' })
          ...
          .then((play) => {
            ...
            // Auto-expand Advanced/Expert sections if loaded play has data in those fields
            const hasAdvancedData = loadedMetadata.concept || loadedMetadata.situation || loadedMetadata.defenseConcept;
            const hasExpertData = loadedMetadata.formation || loadedMetadata.personnel;
            if (hasAdvancedData) {
              setAdvancedExpanded(true);
            }
            if (hasExpertData) {
              setExpertExpanded(true);
            }
    ...
    <div className="h-screen w-screen flex flex-col overflow-hidden">
      ...
      {(isReviewMode || isViewOnly) && fromPlaybookId && (
        <div className={`${isReviewMode ? 'bg-blue-500/90' : 'bg-amber-500/90'} text-white px-4 py-2 flex items-center justify-between`}>
          ...
          <Button
            ...
            onClick={() => {
              // Navigate back to the team playbook with the team pre-selected
              setLocation(`/playbooks?teamId=${fromPlaybookId}`);
            }}
      ...
      <div className="flex flex-row flex-1 gap-4 overflow-hidden">
        <div className={`w-96 min-w-72 flex-shrink rounded-2xl border border-white/10 shadow-2xl overflow-hidden bg-slate-900/95 flex flex-col h-full overflow-y-auto relative ${isViewOnly ? 'opacity-50 pointer-events-none' : ''} ${isReviewMode ? 'review-mode' : ''}`}>
          ...
          <div className="flex-1 overflow-y-auto pt-2 px-4 pb-2 space-y-2">
            ...
            <Card className="p-3 space-y-1.5">
              ...
              {/* Name Field - Always visible, inline layout */}
              <div className="flex items-center gap-2">
                <Label htmlFor="play-name" className="text-xs w-14 flex-shrink-0">Name</Label>
                <Input
                  id="play-name"
              ...
              {advancedExpanded && (
                <div className="space-y-1.5">
                  ...
                  {playType === "defense" && (
                    <div className="flex items-center gap-2">
                      <Label htmlFor="defense-concept" className="text-xs w-14 flex-shrink-0">Concept</Label>
                      <Select value={metadata.defenseConcept} onValueChange={(v) => setMetadata({ ...metadata, defenseConcept: v === "clear_selection" ? "" : v })}>
                        <SelectTrigger id="defense-concept" data-testid="select-defense-concept" className="h-8 text-sm flex-1">
                          <SelectValue placeholder="Concept..." />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="clear_selection">None</SelectItem>
        ...
        <div 
          ...
          className="flex-1 h-full relative rounded-2xl bg-slate-900/50 border border-white/5 overflow-hidden flex flex-col items-center"
          onClick={handleBackgroundClick}
        >
          {/* Layer B: AI Play Creator Interface - Fixed size, centered over field (DOES NOT SCALE) */}
          {(playType === "special" || playType === "ai-beta") && (
            <div 
              className="absolute inset-0 z-20 flex items-start justify-center pt-20 pointer-events-none"
              ...
              {footballs.map((football) => (
                ...
                >
                  ...
                  {football.hasRPO && (
                    ...
                    >
                      <circle cx="10" cy="10" r="10.5" fill="white" stroke="#000" strokeWidth="1.5" />
        ...
        <div className="w-96 border-l border-border bg-card p-4 flex flex-col" data-testid="directions-panel">
          ...
          <div className="space-y-4 text-sm text-muted-foreground flex-1 overflow-y-auto">
            ...
            <div>
              ...
              <p className="pl-3">• 🟠 When outlined orange, click field points to draw</p>
              <p className="pl-3">• 💨 Toggle <span className="font-medium">Motion</span> for pre-snap movement</p>
              <p className="pl-3">• ⭐ Toggle <span className="font-medium">Primary</span> to mark main target</p>
              <p className="italic mt-1">Click route endpoint to finish.</p>
            </div>
